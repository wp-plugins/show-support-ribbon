...
$ssr_plugin  = __('Show Support Ribbon');
...
function ssr_require_wp_version() {
	...
	if (version_compare($wp_version, '3.4', '<')) {
		if (is_plugin_active($ssr_path)) {
			...
			$msg =  '<strong>' . $ssr_plugin . '</strong> ' . __('requires WordPress 3.4 or higher, and has been deactivated!') . '<br />';
...
function ssr_display_ribbon() { 
	...
	if ($enable == true) {
		if ($style == 'ssr_badge') {
			...
			$css_div  = 'position:fixed;right:-2px;top:5px;z-index:9999;';
...
function ssr_plugin_action_links($links, $file) {
	global $ssr_path;
	if ($file == $ssr_path) {
		$ssr_links = '<a href="' . get_admin_url() . 'options-general.php?page=' . $ssr_path . '">' . __('Settings') .'</a>';
...
function ssr_render_form() {
	...
	<div id="mm-plugin-options" class="wrap">
		...
		<div id="mm-panel-toggle"><a href="<?php get_admin_url() . 'options-general.php?page=' . $ssr_path; ?>"><?php _e('Toggle all panels'); ?></a></div>
				...
				<div class="meta-box-sortables ui-sortable">
					<div id="mm-panel-overview" class="postbox">
						...
						<div class="toggle default-hidden">
							<div class="mm-panel-overview">
								<p>
									<strong><?php echo $ssr_plugin; ?></strong> <?php _e('(SSR) displays a custom support ribbon on your site. SSR includes four built-in display styles: badge, banner, ribbon, or link.'); ?>
									<?php _e('SSR also includes the option to use your own custom CSS to style the ribbon and position it anywhere on the page. See the "Customization Tips" panel for more information.'); ?>
									<?php _e('Optionally you may use the shortcode to display the ribbon on any post or page, or use the template tag to display the ribbon anywhere in your theme template.'); ?>
								</p>
								<ul>
									<li><?php _e('To configure SSR, visit the'); ?> <a id="mm-panel-primary-link" href="#mm-panel-primary"><?php _e('Options panel'); ?></a>.</li>
									<li><?php _e('For shortcodes and template tags, visit'); ?> <a id="mm-panel-secondary-link" href="#mm-panel-secondary"><?php _e('Shortcodes &amp; Template Tags'); ?></a>.</li>
									<li><?php _e('For some alternate CSS styles, visit'); ?> <a id="mm-panel-tertiary-link" href="#mm-panel-tertiary"><?php _e('Customization Tips'); ?></a>.</li>
									<li><?php _e('For more information check the <code>readme.txt</code> and'); ?> <a href="<?php echo $ssr_homeurl; ?>"><?php _e('SSR Homepage'); ?></a>.</li>
								</ul>
							</div>
						</div>
					</div>
					<div id="mm-panel-primary" class="postbox">
						...
						<div class="toggle<?php if (!isset($_GET["settings-updated"])) { echo ' default-hidden'; } ?>">
							<p><?php _e('Choose your options..'); ?></p>
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									...
									<tr class="mm-custom-style">
										<th scope="row"><label class="description"><?php _e('HTML'); ?></label></th>
										<td>
											<div><?php _e('<abbr title="For Your Information">FYI</abbr>: here is the markup that is used to create the ribbon:'); ?></div>
...
&lt;/div&gt;</pre>
									...
									<tr class="mm-custom-style">
										<th scope="row"><label class="description" for="ssr_options[ssr_outer]"><?php _e('<code>&lt;div&gt;</code> styles'); ?></label></th>
										...
										<div class="mm-item-caption"><?php _e('Enter some CSS for the <code>&lt;div&gt;</code> tag.'); ?></div></td>
									</tr>
									<tr class="mm-custom-style">
										<th scope="row"><label class="description" for="ssr_options[ssr_inner]"><?php _e('<code>&lt;a&gt;</code> styles'); ?></label></th>
										...
										<div class="mm-item-caption"><?php _e('Enter some CSS for the <code>&lt;a&gt;</code> tag.'); ?></div></td>
									</tr>
									<tr>
										<th scope="row"><label class="description" for="ssr_options[ssr_href]"><?php _e('Link URL'); ?></label></th>
										<td><input type="text" size="50" maxlength="200" name="ssr_options[ssr_href]" value="<?php echo $ssr_options['ssr_href']; ?>" />
										<div class="mm-item-caption"><?php _e('Enter the URL that should be used for the ribbon link.'); ?></div></td>
									</tr>
									<tr>
										<th scope="row"><label class="description" for="ssr_options[ssr_title]"><?php _e('Link title'); ?></label></th>
										<td><input type="text" size="50" maxlength="200" name="ssr_options[ssr_title]" value="<?php echo $ssr_options['ssr_title']; ?>" />
										<div class="mm-item-caption"><?php _e('Enter the title that should be used for the ribbon link.'); ?></div></td>
									</tr>
									<tr>
										<th scope="row"><label class="description" for="ssr_options[ssr_link]"><?php _e('Link text'); ?></label></th>
										<td><input type="text" size="50" maxlength="200" name="ssr_options[ssr_link]" value="<?php echo $ssr_options['ssr_link']; ?>" />
										<div class="mm-item-caption"><?php _e('Enter the anchor text that should be used for the ribbon link.'); ?></div></td>
									</tr>
								</table>
							</div>
					...
					<div id="mm-panel-secondary" class="postbox">
						...
						<div class="toggle<?php if (!isset($_GET["settings-updated"])) { echo ' default-hidden'; } ?>">
							<div class="mm-code-example">
								...
								<div><?php _e('Display the ribbon on any post or page:'); ?></div>
								<div><code>[show_support_ribbon]</code></div>

								<h4><?php _e('Template tag'); ?></h4>
								<div><?php _e('Display the ribbon anywhere in your theme:'); ?></div>
								<div><code>&lt;?php if (function_exists('show_support_ribbon')) show_support_ribbon(); ?&gt;</code></div>
							</div>
						</div>
					</div>
					<div id="mm-panel-tertiary" class="postbox">
						<h3><?php _e('Customization Tips'); ?></h3>
						<div class="toggle<?php if (!isset($_GET["settings-updated"])) { echo ' default-hidden'; } ?>">
							<p><?php _e('For those new to styling HTML with CSS, here are some recipes and tips.'); ?></p>
							<ul>
					...
					<div id="mm-restore-settings" class="postbox">
						...
						<div class="toggle<?php if (!isset($_GET["settings-updated"])) { echo ' default-hidden'; } ?>">
							<p>
								...
								<label class="description" for="ssr_options[default_options]"><?php _e('Restore default options upon plugin deactivation/reactivation.'); ?></label>
							</p>
							<p>
								<small>
